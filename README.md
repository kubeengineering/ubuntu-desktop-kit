...
Здесь другой подход. Каждое изменение записывается блоком с маркером, исходные значения запоминаются до первой правки, а `revert` возвращает ровно то, что было — не «умолчания системы», а именно ваше прежнее состояние. Скрипт умеет объяснить, что он собирается сделать (`--dry-run`), и проверить себя на живой машине (`selftest` — 364 проверки).
...
| Образ | Каким получается |
|---|---|
| [`work`](docs/screenshots/look-work.png) | тёмный: стеклянная панель, виджет справа, прозрачный терминал |
| [`night`](docs/screenshots/look-night.png) | темнее и строже: острые углы, глухая подложка виджета |
| [`paper`](docs/screenshots/look-paper.png) | светлый день: мягкие углы, светлый виджет, непрозрачный терминал |
